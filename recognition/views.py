...
class about(TemplateView):  # new
    template_name = "recognition/about.html"
class contact(TemplateView):  # new
...
def hours_vs_date_given_employee(present_qs,time_qs,admin=True):
	...
	for obj in qs:
		...
		if(obj.time_in is not None and obj.time_out is not None):
			...
			obj.hours=hours
			#fare at 1 INR FOR ONE MINUTE OF RIDE
			obj.fare=round((to-ti).total_seconds()/60, 2)
...
def mark_your_attendance(request):
	...
	while(True):
		...
		#Showing the image in another window
		#Creates a window with window name "Face" and with the image img
		cv2.imshow("Board - In - Press q to exit",frame)
		#Before closing it we need to give a wait command, otherwise the open cv wont work
		# @params with the millisecond of delay 1
		#cv2.waitKey(1)
		#To get out of the loop
...
def mark_your_attendance_out(request):
	...
	while(True):
		...
		#Showing the image in another window
		#Creates a window with window name "Face" and with the image img
		cv2.imshow("Deboard- Out - Press q to exit",frame)
		#Before closing it we need to give a wait command, otherwise the open cv wont work
		# @params with the millisecond of delay 1
		#cv2.waitKey(1)
		#To get out of the loop
...
def view_attendance_home(request):
	...
	return render(request,"recognition/view_attendance_home.html", {'total_num_of_emp' : total_num_of_emp, 'emp_present_today': 1})
